...
const createTransaction = asyncHandler(async (req,res)=>{
   const {user} =  req.user
    ...
    const [transTypeResult] = await pool.query("SELECT * FROM transaction_type WHERE trans_type=?",[type.toUpperCase()])
    const [categoryTypeResult]  = await pool.query("SELECT * FROM category WHERE category=?",[category.toUpperCase()])

    const transType = transTypeResult[0].id
    const categoryType = categoryTypeResult[0].cat_id

    const transactionId = uuidv4() 
    await pool.query("INSERT INTO transactions (id,user_id,amount,type_id,category_id,note) VALUES (?,?,?,?,?,?)",[
...
const getTransactions = asyncHandler(async (req,res)=>{
    ...
    if(Object.keys(req.query).length > 0){
        let query = `
                ...
                `;
            if(type){
                const [transactionType] = await pool.query("SELECT * from transaction_type WHERE trans_type=?",[type.toUpperCase()])
                query += ` AND tt.id=${transactionType[0].id}`
            }
            if(category){
                const [categoryType] = await pool.query("SELECT * from category WHERE category=?",[category.toUpperCase()])
    ...
    const [fetchedTransactions] = await pool.query("SELECT * FROM transactions WHERE user_id=?",[requestedUserId])
